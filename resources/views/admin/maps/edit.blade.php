...
<div class="flex flex-row flex-wrap flex-grow mt-2">
    <div class="w-full p-3">
        ...
        <div class="card card-side bg-base-100 shadow-xl">
            <form action="{{ route('maps.update', $maps->id) }}" method="POST" class="w-full"
                enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <h2 class="card-title">{{ $title }}</h2>
                    <input type="hidden" name="id" value="{{old('id', $maps->id)}}">
        ...
        function initMarkers() {
            ...
            initialMarkers.forEach((data, index) => {
                ...
                <figure class="px-0 pt-0">
                    <img src="/assets/image/mark/${data.position.image}" alt="Shoes" class="rounded-xl" />
                </figure>
